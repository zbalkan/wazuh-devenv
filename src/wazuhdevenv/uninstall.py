...
def _workspace_mount_source(workspace: Path, name: str) -> Path | None:
...
def _active_wazuh_mounts(runner: CommandRunner) -> list[Path]:
    active: list[Path] = []
...
def _preflight_mounts(
    ...
    for name in ("rules", "decoders"):
        ...
        if (
            ...
            continue
        source = _workspace_mount_source(workspace, name)
        if source is None:
            continue
...
def _remove_wazuh_if_unmounted(
    runner: CommandRunner,
    package_manager: PackageManager,
) -> tuple[bool, list[Path]]:
    active_mounts = _active_wazuh_mounts(runner)
    if active_mounts:
        return False, active_mounts
    return _remove_wazuh(runner, package_manager), []
...
def uninstall_environment(home: Path, user: InvokingUser) -> UninstallResult:
    ...
    remaining_mounts = _active_wazuh_mounts(runner)
    if remaining_mounts:
        remnants.append(
            "active Wazuh bind mounts remain mounted: "
            + ", ".join(str(path) for path in remaining_mounts)
        )
    ...
    if installed_by_tool:
        package_removed, active_mounts = _remove_wazuh_if_unmounted(
            runner,
            package_manager,
        )
        if active_mounts:
            mounted = ", ".join(str(path) for path in active_mounts)
            preserved.append(
                "Wazuh package/tree cleanup because retained mounts remain "
                f"active: {mounted}"
            )
            remnants.append(
                "Wazuh package removal and recursive /var/ossec cleanup were "
                "skipped to protect content behind active bind mounts"
            )
        else:
            if package_removed:
