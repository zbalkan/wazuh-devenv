...
def test_mount_cleanup_skips_missing_workspace_source(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    workspace = tmp_path / "workspace"
    ...
    monkeypatch.setattr(
        ...
        lambda *args: (_ for _ in ()).throw(
            AssertionError("missing source must not be stat'ed")
        ),
    )

    runner = MountedRunner()
    uninstall._preflight_mounts(runner, workspace, set())
    assert uninstall._remove_mounts(runner, workspace, set()) == []


def test_wazuh_removal_is_skipped_while_mounts_remain(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    retained = [Path("/var/ossec/etc/rules")]
    monkeypatch.setattr(
        uninstall,
        "_active_wazuh_mounts",
        lambda runner: retained,
    )
    monkeypatch.setattr(
        uninstall,
        "_remove_wazuh",
        lambda *args: (_ for _ in ()).throw(
            AssertionError("Wazuh removal must not run with active mounts")
        ),
    )

    removed, active = uninstall._remove_wazuh_if_unmounted(
        object(),
        object(),
    )
